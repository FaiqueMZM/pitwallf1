...
export default function TrackMap2D({ track, carPositions = [], height = 350, className = '' }: TrackMap2DProps) {
      ...
      fetch(getGeoJsonUrl(track.geojsonId))
        ...
        .then(geojson => {
          ...
          const projection = d3.geoMercator()
            .fitExtent([[PAD, PAD], [W - PAD, H - PAD]], geojson as any)
        ...
        if (firstFeature?.geometry?.coordinates) {
          ...
          if (firstCoord) {
            const [px, py] = projection(firstCoord as [number, number]) ?? [0, 0]
            g.append('circle')
        ...
        if (carPositions.length > 0) {
          const trackPath = svgRef.current?.querySelector('path.track-line') as SVGPathElement | null
          if (trackPath) {
  ...
  return (
    <div className={`relative w-full bg-[#0a0a0a] flex items-center justify-center ${className}`} style={{ height }}>
      {loading && (
      ...
      {error && (
        <div className="absolute inset-0 flex items-center justify-center">
          <p className="text-f1-gray-3 text-xs text-center px-4">
            Couldn't load track map.<br />
            <span className="text-f1-gray-2">Check your internet connection.</span>
          </p>
        </div>
      )}
      <svg
