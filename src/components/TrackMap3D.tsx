...
export default function TrackMap3D({
      ...
      .then((points) => {
        ...
        carPositions.forEach((car) => {
          ...
          const color = new THREE.Color(
            car.teamColour ? `#${car.teamColour.replace("#", "")}` : "#888888",
          );
          ...
          ctx.fillStyle = car.teamColour
            ? `#${car.teamColour.replace("#", "")}`
            : "#888888";
          ctx.beginPath();
          ctx.roundRect(0, 0, 80, 36, 6);
          ctx.fill();
  ...
  return (
    <div
      className={`relative rounded-lg overflow-hidden bg-[#0a0a0a] ${className}`}
      style={{ height }}
    >
      ...
      {error && (
        <div className="absolute inset-0 flex items-center justify-center bg-[#0a0a0a]">
          <p className="text-f1-gray-3 text-xs text-center">
            Couldn't load 3D track map.
            <br />
            <span className="text-f1-gray-2">Check your connection.</span>
          </p>
        </div>
      )}
      {!loading && !error && (
        <div className="absolute top-3 left-3 right-3 flex items-start justify-between pointer-events-none">
          <div className="bg-black/70 backdrop-blur-sm rounded-lg px-3 py-2 border border-white/10">
            <p className="font-display font-bold text-white text-sm uppercase tracking-widest leading-tight">
              {track.circuit}
            </p>
            <p className="text-white/40 text-xs mt-0.5">
              {track.lapLength} km · {track.turns} turns
            </p>
          </div>
          <div className="bg-black/70 backdrop-blur-sm rounded-lg px-3 py-2 border border-white/10 text-right">
            ...
            {carPositions.length > 0 && (
              <p className="text-[#e8002d] text-xs font-semibold mt-0.5">
                {carPositions.length} cars live
              </p>
            )}
          </div>
        </div>
