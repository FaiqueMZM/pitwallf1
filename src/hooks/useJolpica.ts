...
export function useSeasonSchedule(year: number | 'current' = 'current') {
  return useSWR(`schedule-${year}`, () => jolpica.getSeasonSchedule(year), SCHEDULE_OPTS)
}
...
export function useRaceResults(year: number | 'current', round: number | 'last') {
  return useSWR(`results-${year}-${round}`, () => jolpica.getRaceResults(year, round), RACE_OPTS)
}

export function useQualifyingResults(year: number | 'current', round: number | 'last') {
  return useSWR(`qualifying-${year}-${round}`, () => jolpica.getQualifyingResults(year, round), RACE_OPTS)
}

export function useDriverStandings(year: number | 'current' = 'current') {
  return useSWR(`driver-standings-${year}`, () => jolpica.getDriverStandings(year), STANDINGS_OPTS)
}

export function useConstructorStandings(year: number | 'current' = 'current') {
  return useSWR(`constructor-standings-${year}`, () => jolpica.getConstructorStandings(year), STANDINGS_OPTS)
}

export function useAllDrivers(year: number | 'current' = 'current') {
  return useSWR(`drivers-${year}`, () => jolpica.getAllDrivers(year), STANDINGS_OPTS)
}

export function useDriverProfile(driverId: string) {
  return useSWR(`driver-profile-${driverId}`, () => jolpica.getDriverProfile(driverId), STANDINGS_OPTS)
}

export function useDriverSeasonResults(driverId: string, year: number | 'current' = 'current') {
  return useSWR(
