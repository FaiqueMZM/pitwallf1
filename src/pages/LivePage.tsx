...
function getSessionState(session: import('@/types').OpenF1Session | null | undefined): 'live' | 'finished' | 'upcoming' | 'idle' {
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8">
        ...
        {!sessionLoading && !sessionError && (
          <>
            ...
            {/* No active session — show next race + last results */}
            {(sessionState === 'idle' || sessionState === 'upcoming') && (
              <IdleState />
            )}
          </>
        )}
        ...
        <div className="border border-f1-gray/50 rounded-lg p-4 bg-f1-black-2/50">
          <p className="text-f1-gray-3 text-xs leading-relaxed">
            ...
            <a href="https://openf1.org" target="_blank" rel="noopener noreferrer"
              className="text-f1-red hover:underline">OpenF1 API</a>
            {' '}— open-source, community-maintained. Data is delayed approximately 30-60 seconds
            from the live broadcast. Pitwall F1 is not affiliated with Formula 1 or the FIA.
          </p>
        </div>
      </div>
    </div>
...
function SessionBanner({
  ...
  state: 'live' | 'finished' | 'upcoming' | 'idle'
}) {
...
function WeatherStat({ label, value, highlight }: { label: string; value: string; highlight?: boolean }) {
  return (
    <div className="text-center">
      ...
      <p className={cn('font-mono font-bold text-sm', highlight ? 'text-blue-400' : 'text-white')}>
        {value}
      </p>
    </div>
  )
...
function LiveLeaderboard({
  ...
  // Build combined driver+position map
    ...
    <div>
      ...
      <div className="flex items-center justify-between mb-4">
        <div className="f1-accent-bar">
          ...
          <p className="text-f1-gray-4 text-xs mt-0.5">
            {state === 'live'
              ? 'Updating every 10 seconds · ~30-60s behind broadcast'
              : 'Session complete'}
          </p>
        </div>
        {lastUpdated && (
      ...
      {!isLoading && leaderboard.length === 0 && (
        <div className="text-center py-16 text-f1-gray-4 text-sm">
          No position data available for this session yet.
        </div>
      )}
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-f1-gray">
                  ...
                  <Th className="text-right hidden md:table-cell">Number</Th>
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="px-4 py-3.5 hidden sm:table-cell">
                        <span className="text-f1-gray-4 text-sm" style={{ color: teamColor }}>
                          {driver.team_name}
                        </span>
                      </td>
              ...
              <table className="w-full text-sm">
                <tbody>
                  {lastRace.Results.slice(0, 10).map((result) => {
                    ...
                    const podiumColor = podiumColors[pos]
                      ...
                      <tr key={result.Driver.driverId} className="f1-table-row">
                        <td className="px-4 py-2.5 w-10">
                          <span
                            ...
                            style={{ color: podiumColor ?? '#555555' }}
                          >
                            {pos}
                          </span>
                        </td>
