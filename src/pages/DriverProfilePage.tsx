...
import { useDriverProfile, useDriverSeasonResults, useDriverStandings, useDriverSeasons } from '@/hooks/useJolpica'
...
export default function DriverProfilePage() {
  ...
  const { data: driver, error: driverError, isLoading: driverLoading } = useDriverProfile(driverId ?? '')
  ...
  const { data: results, isLoading: resultsLoading } = useDriverSeasonResults(driverId ?? '', activeYear)
  ...
  const wins = results?.filter((r) => r.Results?.[0]?.position === '1').length ?? 0
  const podiums = results?.filter((r) => {
    const pos = parseInt(r.Results?.[0]?.position ?? '99')
    return pos <= 3
  }).length ?? 0
  const dnfs = results?.filter((r) => r.Results?.[0]?.status !== 'Finished' &&
    !r.Results?.[0]?.status?.startsWith('+')
  ).length ?? 0
  const totalPoints = results?.reduce((sum, r) =>
    sum + parseFloat(r.Results?.[0]?.points ?? '0'), 0
  ) ?? 0
  ...
  return (
    <div className="animate-fade-in">
      <PageHeader
        ...
        subtitle={standing ? `${standing.Constructors[0]?.name} · ${activeYear} Championship P${standing.position}` : driver.nationality}
        badge={
          <div className="flex items-center gap-2">
            <Link to="/drivers" className="text-f1-gray-4 hover:text-white text-xs transition-colors">
              ← Drivers
            </Link>
            {driver.code && (
              <>
                <span className="text-f1-gray-2">·</span>
                <span className="font-mono text-f1-gray-4 text-xs tracking-widest">{driver.code}</span>
              </>
            )}
            {driver.permanentNumber && (
              <>
                <span className="text-f1-gray-2">·</span>
                <span className="font-mono text-f1-gray-4 text-xs">#{driver.permanentNumber}</span>
              </>
            )}
          </div>
        }
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="f1-card overflow-hidden">
            ...
            <div className="p-6">
              ...
              <p className="font-display text-4xl font-bold uppercase leading-tight" style={{ color: teamColor }}>
                {driver.familyName}
              </p>
              <div className="mt-4 space-y-2">
                <InfoRow label="Nationality" value={`${getNationalityFlag(driver.nationality)} ${driver.nationality}`} />
                <InfoRow label="Date of birth" value={formatRaceDate(driver.dateOfBirth)} />
                {standing && <InfoRow label="Team" value={standing.Constructors[0]?.name ?? '—'} />}
                {standing && <InfoRow label="Championship" value={`P${standing.position} · ${standing.points} pts`} />}
              </div>
            </div>
          </div>
          ...
          <div className="lg:col-span-2 grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-2 xl:grid-cols-4 gap-4">
            {[
              { label: 'Points', value: totalPoints.toString(), sub: `${activeYear} season` },
              { label: 'Wins', value: wins.toString(), sub: `${activeYear} season` },
              { label: 'Podiums', value: podiums.toString(), sub: `${activeYear} season` },
              { label: 'DNFs', value: dnfs.toString(), sub: `${activeYear} season` },
            ].map(({ label, value, sub }) => (
              <div key={label} className="f1-card p-5">
                <p className="text-f1-gray-3 text-xs uppercase tracking-widest font-semibold mb-2">{label}</p>
            ...
            {standing && (
              <div className="f1-card p-5 col-span-2 sm:col-span-4 lg:col-span-2 xl:col-span-4">
                <div className="flex items-center justify-between mb-3">
                  ...
                  <span className="font-mono text-sm text-f1-gray-4">P{standing.position}</span>
                </div>
                {(() => {
                  const leaderPts = parseFloat(standings?.[0]?.points ?? '0')
                  const driverPts = parseFloat(standing.points)
                  const pct = leaderPts > 0 ? (driverPts / leaderPts) * 100 : 100
                  ...
                  return (
                    <>
                      ...
                      <div className="flex justify-between text-xs font-mono text-f1-gray-3">
                        ...
                        {gap === 0 && <span className="text-yellow-400">Championship leader</span>}
                      </div>
                    </>
                  )
                })()}
        ...
        <div className="f1-card p-6">
          <div className="flex flex-wrap items-center justify-between gap-4 mb-6">
            <div>
              <h3 className="section-heading text-xl f1-accent-bar">Race Results</h3>
              <p className="text-f1-gray-4 text-sm mt-1">Finishing position by round</p>
            </div>
            ...
            <div className="flex flex-wrap items-center gap-2">
              ...
              {(seasons ?? []).length > 8 && (
                <select
                  value={(seasons ?? []).slice(0, 8).includes(activeYear) ? '' : activeYear}
                  onChange={(e) => e.target.value && setChartYear(parseInt(e.target.value))}
        ...
        {results && results.length > 0 && (
          <div>
            <h3 className="section-heading text-xl mb-4 f1-accent-bar">{activeYear} Race Log</h3>
            <div className="f1-card overflow-hidden">
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr className="border-b border-f1-gray">
                      ...
                      <Th className="text-right hidden sm:table-cell">Status</Th>
                  ...
                  <tbody>
                    {results.map((race) => {
                      ...
                      const podiumColor = pos <= 3 ? getPodiumColor(pos) : undefined

                      return (
                        <tr key={race.round} className="f1-table-row group">
                          <td className="px-4 py-3">
                            <span className="font-mono text-xs text-f1-gray-3">R{race.round}</span>
                          </td>
                          <td className="px-4 py-3">
                            <Link
                            ...
                            <p className="text-f1-gray-3 text-xs">{formatRaceDate(race.date)}</p>
                          </td>
                          ...
                          <td className="px-4 py-3 text-right font-mono font-bold text-white text-sm">
                            {result.points !== '0' ? result.points : (
                              <span className="text-f1-gray-3 font-normal">—</span>
                            )}
                          </td>
                        </tr>
                      )
...
function InfoRow({ label, value }: { label: string; value: string }) {
  return (
    <div className="flex items-center justify-between gap-4 py-1.5 border-b border-f1-gray/50 last:border-0">
      <span className="text-f1-gray-3 text-xs uppercase tracking-wider font-semibold">{label}</span>
