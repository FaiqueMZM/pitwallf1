...
function Podium({ results }: { results: RaceResult[] }) {
  // Reorder to P2 P1 P3 for visual podium
          ...
          return (
            <div key={result.Driver.driverId} className="flex flex-col items-center">
              ...
              <div className="text-center mb-3 px-2">
                <p className="text-f1-gray-4 text-xs mb-1">
                  {getNationalityFlag(result.Driver.nationality)}
                </p>
                <p className="font-display font-bold text-white text-sm uppercase leading-tight">
                  {result.Driver.givenName}
                </p>
                <p
                  className="font-display font-bold text-base uppercase leading-tight"
                  style={{ color: teamColor }}
                >
                  {result.Driver.familyName}
                </p>
                <p className="text-f1-gray-3 text-xs mt-1">{result.Constructor.name}</p>
                <p className="text-f1-white-2 font-mono text-xs mt-1 font-semibold">
              ...
              {/* Podium block */}
              <div
                className={`w-24 sm:w-32 ${heights[i]} rounded-t-lg flex items-center justify-center relative overflow-hidden`}
              ...
              >
                <span
                  ...
                  style={{ color: podiumColor, opacity: 0.8 }}
                >
                  {pos}
                </span>
              </div>
            ...
            {results.map((result) => {
              ...
              return (
                <tr key={result.Driver.driverId} className="f1-table-row">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <div>
                        <div className="flex items-center gap-1.5">
                          ...
                          {result.Driver.code && (
                            <span className="text-f1-gray-3 text-xs font-mono hidden sm:inline">
                              {result.Driver.code}
                            </span>
                          )}
                        </div>
                        <span className="text-f1-gray-3 text-xs">
                          {getNationalityFlag(result.Driver.nationality)} #{result.number}
                        </span>
                      </div>
                    </div>
                  </td>
                  ...
                  <td className="px-4 py-3 hidden md:table-cell text-right font-mono text-f1-gray-4 text-sm">
                  ...
                  <td className="px-4 py-3 text-right">
                    {result.status === 'Finished' || result.Time ? (
                      <span className="font-mono text-f1-white-2 text-sm">
                  ...
                  <td className="px-4 py-3 hidden sm:table-cell text-right">
                    {hasFastestLap && (
                      <span title={`Fastest lap: ${result.FastestLap?.Time.time}`}>
                        <Badge variant="red">FL</Badge>
                      </span>
                    )}
                  </td>
                  <td className="px-4 py-3 text-right font-mono font-bold text-f1-white-2 text-sm">
                    {result.points !== '0' ? result.points : (
                      <span className="text-f1-gray-3">—</span>
                    )}
                  </td>
                </tr>
              )
                ...
                <tr key={result.Driver.driverId} className="f1-table-row">
                  <td className="px-4 py-3">
                    <span className={cn(
                      'font-mono text-sm font-bold',
                      pos === 1 ? 'text-yellow-400' :
                      pos <= 3 ? 'text-f1-white-2' : 'text-f1-gray-4'
                    )}>
                      {pos}
                    </span>
                  </td>
                  ...
                  <td className="px-4 py-3 text-right font-mono text-f1-gray-4 text-sm">
                    {result.Q1 ?? <span className="text-f1-gray-2">—</span>}
                  </td>
                  <td className="px-4 py-3 text-right font-mono text-f1-gray-4 text-sm hidden sm:table-cell">
                    {result.Q2 ?? <span className="text-f1-gray-2">—</span>}
                  </td>
                  <td className="px-4 py-3 text-right font-mono text-f1-white-2 text-sm font-semibold hidden md:table-cell">
                    {result.Q3 ?? <span className="text-f1-gray-3 font-normal">—</span>}
                  </td>
                </tr>
              )
            })}
